...
const BASE_URL: &str = "https://animesdigital.org";

pub struct AnimesdigitalClient {
    client: reqwest::Client,
}

impl AnimesdigitalClient {
    ...
    pub async fn post_form(
        ...
        let response = self
            .client
            .post(&url)
            .headers(Self::stream_headers(Some(referer)))
            .form(form)
            .send()
            .await?
            .error_for_status()?;
    ...
    pub fn is_episode_url(url: &str) -> bool {
        let normalized = Self::normalize_url(url);
        normalized.contains("/video/a/") || normalized.contains("/video/")
    }

    pub fn is_anime_url(url: &str) -> bool {
        let normalized = Self::normalize_url(url);
        (normalized.contains("/anime/b/") || normalized.contains("/anime/"))
            && !Self::is_episode_url(&normalized)
    }

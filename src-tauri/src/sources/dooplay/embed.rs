...
use crate::sources::shared::stream::{
    ...
    extract_iframe_srcs, extract_m3u8_from_query, parse_blogger_video_config, parse_stream_from_html,
...
async fn resolve_blogger_iframe(
    ...
    let page = client.fetch_url(iframe_url, Some(referer)).await?;

    let parsed = parse_blogger_video_config(&page)

        .or_else(|| parse_stream_from_html(&page).ok())
...
async fn resolve_meusdoramas_iframe(
    ...
    let data: MeusdoramasVideoResponse = serde_json::from_str(&body).map_err(|e| SourceError::Parse(e.to_string()))?;
